...
namespace algoim
{
    namespace detail
    {
        ...
        template <typename xdouble, int N>
        void eliminate_axis(PolySet<xdouble, N, ALGOIM_M> &phi, int k, PolySet<xdouble, N - 1, ALGOIM_M> &psi)
        {
    ...
    struct ImplicitPolyQuadrature
    {
        ...
        PolySet<xdouble, N, ALGOIM_M> phi;                                                     // Given N-dimensional polynomials
        ...
        void integrate(QuadStrategy strategy, int q, const F &f)
        {
            ...
            if (k == N)
            {
                ...
                std::cout << "There are no cut interfaces, apply rectangular tensor-product Gauss-Legendre quadrature. " << std::endl;
                return;
            }
            ...
            auto integrand = [&](const uvector<real, N - 1> &xbase, real w)
            {
                ...
                xdouble *nodes;
                algoim_spark_alloc(xdouble, &nodes, max_count);
                nodes[0] = 0.0;
                nodes[1] = 1.0;
                int count = 2;
                // For every phi(i) ...
                ...
                for (int i = 0; i < count - 1; ++i)
                {
                    ...
                    if (GL)
                        for (int j = 0; j < q; ++j)
                        {
                            ...
                            xdouble xq = (x0 + (x1 - x0) * GaussQuad::x(q, j));
                            f(xq, w * (x1 - x0) * GaussQuad::w(q, j));
                        }
                    else
                        for (int j = 0; j < q; ++j)
                        {
    ...
    template <>
    struct ImplicitPolyQuadrature<double, 0>
    {
    };
    template <>
    struct ImplicitPolyQuadrature<duald, 0>
    {
    };
} // namespace algoim
