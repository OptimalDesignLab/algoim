...
template<typename xdouble, int N, typename Phi, typename F>
void qConv(const Phi& phi, xdouble xmin, xdouble xmax, uvector<int,N> P, const F& integrand, int qMax, xdouble volume_exact, xdouble surf_exact)
{
    // Construct Bernstein polynomial by mapping [0,1] onto bounding box [xmin,xmax]
    xarray<xdouble,N> phipoly(nullptr, P);
    algoim_spark_alloc(xdouble, phipoly);
    bernstein::bernsteinInterpolate<xdouble, N>([&](const uvector<xdouble,N>& x) { return phi(xmin + x * (xmax - xmin)); }, phipoly);

    // Build quadrature hierarchy
    ImplicitPolyQuadrature<xdouble, N> ipquad(phipoly);
    ...
    auto compute = [&](int q)
    {
        ...
        ipquad.integrate(AutoMixed, q, [&](const uvector<xdouble,N>& x,xdouble w)
        {
            if (bernstein::evalBernsteinPoly<xdouble>(phipoly, x) < 0)
                volume += w * integrand(xmin + x * (xmax - xmin));
        });
        // compute surface integral over {phi == 0} using AutoMixed strategy
        ipquad.integrate_surf(AutoMixed, q, [&](const uvector<xdouble,N>& x, xdouble w, const uvector<xdouble,N>& wn)
        {
    ...
    {
        compute(q);
        std::cout << q << ' ' << volume << ' ' << surf << ' ' << fabs(volume - volume_exact)/volume_exact << ' ' << fabs(surf - surf_exact)/surf_exact << std::endl;
    }
}
...
void outputQuadratureRuleAsVtpXML(const std::vector<uvector<xdouble,N+1>>& q, std::string fn)
{
...
void outputQuadScheme(const F& fphi, xdouble xmin, xdouble xmax, const uvector<int,N>& P, int q, std::string qfile)
{
    // Construct phi by mapping [0,1] onto bounding box [xmin,xmax]
    xarray<xdouble,N> phi(nullptr, P);
    algoim_spark_alloc(xdouble, phi);
    bernstein::bernsteinInterpolate<N>([&](const uvector<xdouble,N>& x) { return fphi(xmin + x * (xmax - xmin)); }, phi);

    // Build quadrature hierarchy
    ImplicitPolyQuadrature<xdouble, N> ipquad(phi);
    ...
    std::vector<uvector<xdouble,N+1>> phase0, phase1, surf;
    ipquad.integrate(AutoMixed, q, [&](const uvector<xdouble,N>& x, xdouble w)
    {
...
// given q, and outputs it for visualisation in a set of VTP XML files
template<typename xdouble, int N, typename F1, typename F2>
void outputQuadScheme(const F1& fphi1, const F2& fphi2, xdouble xmin, xdouble xmax, const uvector<int,N>& P, int q, std::string qfile)
{
    // Construct phi by mapping [0,1] onto bounding box [xmin,xmax]
    xarray<xdouble,N> phi1(nullptr, P), phi2(nullptr, P);
    algoim_spark_alloc(xdouble, phi1, phi2);
    bernstein::bernsteinInterpolate<N>([&](const uvector<xdouble,N>& x) { return fphi1(xmin + x * (xmax - xmin)); }, phi1);
    bernstein::bernsteinInterpolate<N>([&](const uvector<xdouble,N>& x) { return fphi2(xmin + x * (xmax - xmin)); }, phi2);

    // Build quadrature hierarchy
    ImplicitPolyQuadrature<xdouble, N> ipquad(phi1, phi2);
    ...
    std::vector<uvector<xdouble,N+1>> vol, surf;
    ipquad.integrate(AutoMixed, q, [&](const uvector<xdouble,N>& x, xdouble w)
    {
...
int main(int argc, char* argv[])
{
    ...
    {
        auto ellipse = [](const uvector<adouble,2>& x)
        {
            return x(0)*x(0) + x(1)*x(1)*4 - 1;
        };
        auto integrand = [](const uvector<adouble,2>& x)
        {
            return 1.0;
        };
        adouble volume_exact = algoim::util::pi / 2;
        adouble surf_exact = 4.844224110273838099214251598195914705976959198943300412541558176231060;
        std::cout << "\n\nEllipse q-convergence test\n";
        std::cout << "q      area(q)         perim(q)        area error       perim error\n";
        qConv<adouble, 2>(ellipse, -1.1, 1.1, 3, integrand, 50, volume_exact, surf_exact);
    }

    // q-convergence study for a 3D ellipsoid
    {
        auto ellipsoid = [](const uvector<adouble,3>& x)
        {
            return x(0)*x(0) + x(1)*x(1)*4 + x(2)*x(2)*9 - 1;
        };
        auto integrand = [](const uvector<adouble,3>& x)
        {
            return 1.0;
        };
        adouble volume_exact = (algoim::util::pi * 2) / 9;
        adouble surf_exact = 4.400809564664970341600200389229705943483674323377145800356686868037845;
        std::cout << "\n\nEllipsoid q-convergence test\n";
        std::cout << "q      volume(q)         surf(q)        vol error       surf error\n";
        qConv<adouble, 3>(ellipsoid, -1.1, 1.1, 3, integrand, 50, volume_exact, surf_exact);
    }
    ...
    {
        auto phi = [](const uvector<adouble,2>& xx)
        {
            adouble x = xx(0)*2 - 1;
            adouble y = xx(1)*2 - 1;
        ...
        outputQuadScheme<adouble, 2>(phi, 0.0, 1.0, 3, 3, "exampleA");
    ...
    {
        auto phi = [](const uvector<adouble,3>& xx)
        {
            adouble x = xx(0)*2 - 1;
            adouble y = xx(1)*2 - 1;
            adouble z = xx(2)*2 - 1;
        ...
        outputQuadScheme<adouble, 3>(phi, 0.0, 1.0, 3, 3, "exampleB");
    ...
    {
        auto phi0 = [](const uvector<adouble,2>& xx)
        {
            adouble x = xx(0)*2 - 1;
            adouble y = xx(1)*2 - 1;
        ...
        auto phi1 = [](const uvector<adouble,2>& xx)
        {
            adouble x = xx(0)*2 - 1;
            adouble y = xx(1)*2 - 1;
        ...
        outputQuadScheme<adouble, 2>(phi0, phi1, 0.0, 1.0, 3, 3, "exampleC");
