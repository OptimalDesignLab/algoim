        ...
        // q-convergence study for a 2D ellipse
        {
            ...
            auto phi = [&](const uvector<real, 2> &xs)
            {
                ...
                for (int k = 0; k < nbnd; ++k)
                {
                    ...
                    x(0) = a * cos(theta) + xc;
                    x(1) = b * sin(theta) + yc;
                    nrm(0) = 2.0 * (x(0) - xc)/ (a * a);
                    nrm(1) = 2.0 * (x(1) - yc)/ (b * b);
                    ...
                    uvector<real, N> dx, d2x;
                    dx(0) = -a * sin(theta);
                    dx(1) =  b * cos(theta);
                    d2x(0) = -a * cos(theta);
                    d2x(1) = -b * sin(theta);
                    double num = (dx(0) * d2x(1) - dx(1) * d2x(0));
                    double mag_dx = sqrt(sqrnorm(dx));
                    double den = mag_dx * mag_dx * mag_dx;
                    uvector<real,  N - 1> curv;
                    curv(0) = num / den;
            ...
            for (int i = 0; i < nel; ++i)
                for (int j = 0; j < nel; ++j)
                {
                    ...
                    xmin(0) = min_x + i * dx;
                    xmin(1) = min_y + j * dy;
                    xmax(0) = min_x + i * dx + dx;
                    xmax(1) = min_y + j * dy + dy;
